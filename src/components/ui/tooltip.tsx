...
const Tooltip: React.FC<TooltipProps> = ({ children, open: controlledOpen, defaultOpen = false, onOpenChange }) => {
  ...
  return <TooltipContext.Provider value={{ open, setOpen, triggerRef }}>{children}</TooltipContext.Provider>;
};
...
const TooltipContent = React.forwardRef<HTMLDivElement, TooltipContentProps>(
  ({ className, sideOffset = 4, side = 'top', align = 'center', collisionPadding = 8, children, ...props }, ref) => {
    const context = React.useContext(TooltipContext);
    ...
    const computePosition = React.useCallback(() => {
      ...
      if (side === 'bottom' && r.bottom + sideOffset + c.height + collisionPadding > viewportH) chosenSide = 'top';
      else if (side === 'top' && r.top - sideOffset - c.height - collisionPadding < 0) chosenSide = 'bottom';
      else if (side === 'right' && r.right + sideOffset + c.width + collisionPadding > viewportW) chosenSide = 'left';
      else if (side === 'left' && r.left - sideOffset - c.width - collisionPadding < 0) chosenSide = 'right';
