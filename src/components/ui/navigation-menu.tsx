...
// Contexto para controlar qual item está aberto
...
// Contexto para compartilhar o id entre Trigger e Content dentro do mesmo Item
  ...
  ({ className, children, ...props }, ref) => {
    ...
    React.useEffect(() => {
      ...
      const onDocClick = (e: MouseEvent) => {
        ...
        if (!target) return;
        // Se o clique ocorrer dentro do conteúdo portado (data-nav-content), não fechar
    ...
    // Fecha com pequeno atraso ao sair com o mouse
    ...
    return (
      <NavigationMenuContext.Provider value={{ openItemId, setOpenItemId }}>
        ...
        >
          {children}
          {/* Mantemos o viewport para compatibilidade visual, embora o conteúdo fique dentro de cada Item */}
    ...
    <button
      ...
      onKeyDown={(e) => {
        ...
        if (e.key === 'ArrowRight') {
          ...
          // abre o menu ao pressionar seta para baixo
          e.preventDefault();
          handleOpen();
        }
      }}
...
const NavigationMenuContent = React.forwardRef<HTMLDivElement, NavigationMenuContentProps>(
  ({ className, collisionPadding = 8, offsetY = 6, align = 'center', ...props }, ref) => {
    ...
    const clampToViewport = React.useCallback(() => {
      ...
      else desiredX = triggerRect.left + (triggerRect.width - rect.width) / 2; // center
    ...
    const node = (
      <div
        ...
        data-nav-content
        className={cn(
          // Painel do conteúdo: posicionamento fixo com estilo de popover
    ...
    // Renderiza em portal para garantir que fique acima de outros elementos e fora de contextos de empilhamento
...
const navigationMenuLinkVariants = cva('', {
  variants: {
    variant: {
      // Usado para links colocados diretamente na barra (estilo botão/trigger)
      inline: navigationMenuTriggerStyle(),
      // Usado para links dentro do conteúdo do menu (cards com título + descrição)
      content:
