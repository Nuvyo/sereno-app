...
const DropdownMenuTrigger = React.forwardRef<HTMLButtonElement, DropdownMenuTriggerProps>(
  ({ asChild, onClick, children, ...props }, ref) => {
    ...
    return (
      <button
        ref={(node) => {
          (triggerRef as React.MutableRefObject<HTMLElement | null>).current = node as HTMLElement | null;
  ...
  ({ className, side = 'bottom', align = 'start', sideOffset = 8, collisionPadding = 8, ...props }, ref) => {
    ...
    const clampToViewport = React.useCallback(() => {
      const pad = (sideName: 'top' | 'right' | 'bottom' | 'left') =>
        typeof collisionPadding === 'number' ? collisionPadding : (collisionPadding?.[sideName] ?? 8);
      ...
      const belowY = tr.bottom + sideOffset;
      const aboveY = tr.top - sideOffset - rect.height;
      const fitsBelow = belowY + rect.height + pad('bottom') <= vh;
      const desiredY = (side === 'bottom' ? fitsBelow : false) ? belowY : Math.max(pad('top'), aboveY);
