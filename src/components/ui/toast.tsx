...
interface ToastContextValue {
  toasts: Array<{ id: string; [key: string]: unknown }>;
}

const ToastContext = React.createContext<ToastContextValue>({ toasts: [] });
const ToastProvider = ({ children }: { children: React.ReactNode }) => {
  const [toasts] = React.useState<Array<{ id: string; [key: string]: unknown }>>([]);
...
const toastVariants = cva(
  'group pointer-events-auto relative flex w-full items-center justify-between space-x-4 overflow-hidden rounded-md border p-6 pr-8 shadow-lg transition-all animate-in fade-in-80 slide-in-from-top-full',
  {
    variants: {
      variant: {
...
const Toast = React.forwardRef<HTMLDivElement, ToastProps>(({ className, variant, open, ...props }, ref) => {
